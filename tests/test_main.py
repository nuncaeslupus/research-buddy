...
class TestInit:
    ...
    def test_created_file_has_rb_version(self, tmp_path: Path) -> None:
        ...
        with open(doc_path) as f:
            doc = json.load(f)
        assert doc["meta"].get("research_buddy_version") == "1.0"
...
class TestBuild:
    ...
    def test_build_custom_output(self, tmp_project: Path) -> None:
        result = cmd_build(_Args(paths=[str(tmp_project)], output="my-docs.html"))
        assert result == 0
    ...
    def test_build_unversioned_template(self, tmp_path: Path) -> None:
        ...
        assert result == 0
        assert (target / "docs.html").exists()
